...
Este guia resume os passos para configurar uma conta de teste local/admin (`admin@admin`) vinculada à organização "Hybex" e gerar um executável portátil do Nexus para rodar em qualquer máquina sem instalador.
...
O Supabase em produção exige confirmação de e-mail e rejeita formatos sem TLD (como `admin@admin`).
- **Solução no Frontend:** No `AuthContext.tsx`, quando a credencial digitada for `admin@admin` / `admin@admin`, o sistema intercepta o login e redireciona para um perfil mock local ou mapeia internamente para o e-mail real `admin@admin.com`.
...
Interceptar as credenciais `admin@admin` no método `signIn` para retornar uma sessão associada à organização "Hybex".
...
2. Faça login informando `admin@admin` / `admin@admin`.
